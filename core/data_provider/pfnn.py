...
class InputHandle:
    ...
    def get_batch(self):
        ...
        alpha_batch = None if self.alpha is None else self.alpha[slc]
        n_batch = None if self.n_value is None else self.n_value[slc]
        theta_r_batch = None if self.theta_r is None else self.theta_r[slc]
        theta_s_batch = None if self.theta_s is None else self.theta_s[slc]
        porosity_batch = None if self.porosity is None else self.porosity[slc]
        specific_storage_batch = None if self.specific_storage is None else self.specific_storage[slc]
        mask_batch = None if self.mask is None else self.mask[slc]
        ...
        return (
            ...
            maybe_to_device(alpha_batch, self.configs.device),
            maybe_to_device(n_batch, self.configs.device),
            maybe_to_device(theta_r_batch, self.configs.device),
            maybe_to_device(theta_s_batch, self.configs.device),
            maybe_to_device(porosity_batch, self.configs.device),
            maybe_to_device(specific_storage_batch, self.configs.device),
            maybe_to_device(mask_batch, self.configs.device),
        )
